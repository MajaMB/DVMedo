...
if (!function_exists('sectox_loop_product_thumbnail')) {
    function sectox_loop_product_thumbnail()
    {
        ...
        $prating_show = get_theme_mod('prating_show', true);
        $pquick_show = get_theme_mod('pquick_show', true);
        $pwishlist_show = get_theme_mod('pwishlist_show', true);
        $pcart_show = get_theme_mod('pcart_show', true);
        ...
        <div class="product__item text-center transition-3 mb-40">
            <div class="product__thumb p-relative fix">
                ...
                <div class="product__icon">
                    <?php if (!empty($pquick_show)) : ?>
                        <?php if (class_exists('WPCleverWoosq')) {
                            echo do_shortcode('[woosq]');
                        } ?>
                    <?php endif; ?>
                    <?php if (!empty($pcart_show)) : ?>
                        <?php woocommerce_template_loop_add_to_cart(); ?>
                    <?php endif; ?>
                    <?php if (!empty($pwishlist_show)) : ?>
                        <?php if (class_exists('TInvWL_Admin_TInvWL')) {
                            echo do_shortcode('[ti_wishlists_addtowishlist]');
                        } ?>
                    <?php endif; ?>
                </div>
...
function sectox_woo_rating()
{
    ...
    $html   = '';
    $html   .= '<div class="details-rating mb-10 d-none" title="' . $review . '">';
    for ($i = 0; $i <= 4; $i++) {
        if ($i < floor($rating)) {
