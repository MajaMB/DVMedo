...
<div class="tutor-price-preview-box">
        ...
        if ( $wp_query->query['post_type'] !== 'lesson') {
            ...
            if ( $lesson_url ) { 
                ...
                <a href="<?php echo esc_url($lesson_url); ?>" class="<?php echo esc_attr($button_class); ?>" data-course_id="<?php echo get_the_ID(); ?>">
                    <?php
                        if($retake_course) {
                            _e( 'Retake This Course', 'kindedo' );
                        } else if( $completed_percent <= 0 ){
                            _e( 'Start Course', 'kindedo' );
                        } else {
                            _e( 'Continue Course', 'kindedo' );
                        }
                    ?>
                </a>
                <?php 
    ...
    <?php do_action('kindedo_course_enroll_info_list'); ?>
    ...
    <div class="tutor-single-course-segment  tutor-course-enrolled-wrap">
        <p>
            ...
            <?php
                ...
                echo sprintf(__('You have been enrolled on %s.', 'kindedo'),  "<span>". date_i18n(get_option('date_format'), strtotime($enrolled->post_date)
                    )."</span>"  );
                ?>
        </p>
